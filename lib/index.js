...
function resolveActivate(configActivate) {
    if (configActivate === 'none')
        return null;
    if (configActivate)
        return configActivate;
...
function abortMessage(data) {
    ...
    if (reason && typeof reason?.message === 'string' && reason.message.trim()) {
        return compactText(reason.message, 120);
    }
    return '回合已被中止';
}
function errorMessage(data) {
    ...
    if (message)
        return compactText(`发生异常：${message}`, 160);
    return '发生异常，回合终止';
}
...
`;
    try {
        const result = spawnSync('/usr/bin/osascript', ['-e', script, content.message, content.title, content.subtitle, content.sound], { timeout: 5000, stdio: 'ignore' });
        return result.status === 0;
    }
    catch {
        return false;
...
function sendNotification(notifier, content, isPermission, icon, activate) {
    if (notifier) {
        ...
        if (sendWithTerminalNotifier(notifier, content, icon, activate))
            return;
    }
    // 降级到 osascript
    ...
    ctx.on('session/event', (session, event) => {
        ...
        try {
            if (eventType === 'turn/end') {
                ...
                if (!shouldNotifyTurn(kind, events))
                    return;
            ...
            if (eventType === 'approval/decided') {
                ...
                const kind = outcome === 'rejected' ? 'rejected' :
                    outcome === 'cancelled' ? 'cancelled' :
                        outcome === 'unavailable' ? 'unavailable' : null;
                if (!kind || !events.permissionDenied)
                    return;
