...
function buildNotification(eventType, eventData, cwd, config) {
...
export function apply(ctx, config) {
    ...
    ctx.on('session/event', (session, event) => {
        ...
        // 回合完成：turn/end
        // 权限申请：approval/asked
        if (eventType !== 'turn/end' && eventType !== 'approval/asked')
            return;
        const cwd = session.header?.cwd ?? process.cwd();
        try {
            const content = buildNotification(eventType, event.data ?? {}, cwd, config);
